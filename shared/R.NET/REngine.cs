...
using RDotNet.Internals.ALTREP;
...
namespace RDotNet
{
   ...
   /// </summary>
   /// <example>
   ///    This example generates and outputs five random numbers from standard normal distribution.
   ///    <code>
   ...
   public class REngine : UnmanagedDll
   {
      ...
      /// <summary>
      ///    Gets a reference to the R engine, creating and initializing it if necessary. In most cases users need not provide
      ///    any parameter to this method.
      /// </summary>
      /// <param name="dll">
      ///    The file name of the library to load, e.g. "R.dll" for Windows. You usually do not need need to
      ...
      ///    If 'initialize' is 'true', you can optionally specify the specific startup parameters for the R
      ///    native engine
      /// </param>
      ...
      /// <param name="dll">
      ///    The file name of the library to load, e.g. "R.dll" for Windows. You should usually not provide this
      ///    optional parameter
      /// </param>
      /// <returns>The engine.</returns>
         ...
         if (sexprecType == null)
         {
            switch (Compatibility)
            {
               case CompatibilityMode.ALTREP:
                  sexprecType = typeof(SEXPREC);
                  break;
               case CompatibilityMode.PreALTREP:
                  sexprecType = typeof(Internals.PreALTREP.SEXPREC);
                  break;
               default:
      ...
      public Type GetSymSxpType()
      {
         if (symsxpType == null)
         {
            ...
            {
               case CompatibilityMode.ALTREP:
                  symsxpType = typeof(symsxp);
                  break;
               case CompatibilityMode.PreALTREP:
                  symsxpType = typeof(Internals.PreALTREP.symsxp);
                  break;
               default:
      ...
      public Type GetVectorSexprecType()
      {
         if (vectorSexprecType == null)
         {
            ...
            {
               case CompatibilityMode.ALTREP:
                  vectorSexprecType = typeof(VECTOR_SEXPREC);
                  break;
               case CompatibilityMode.PreALTREP:
                  vectorSexprecType = typeof(Internals.PreALTREP.VECTOR_SEXPREC);
                  break;
               default:
                  throw new InvalidCastException("No symsxp type is available for this compatibility level");
            }
      ...
      /// <summary>
      ///    if the parameter is null or empty string, return the default names of the R shared library file depending on the
      ///    platform
      /// </summary>
      ...
      /// </summary>
      /// <param name="rPath">
      ///    The path of the directory containing the R native library.
      ///    If null (default), this function tries to locate the path via the Windows registry, or commonly used locations on
      ///    MacOS and Linux
      /// </param>
      /// <param name="rHome">
      ///    The path for R_HOME. If null (default), the function checks the R_HOME environment variable. If none is set,
      ///    the function uses platform specific sensible default behaviors.
      /// </param>
      /// <remarks>
      ///    This function has been designed to limit the tedium for users, while allowing custom settings for unusual
      ///    installations.
      /// </remarks>
      ...
      ///    to get the startup parameters taken into account. Passing the StartupParameter to the API seems not to work as
      ///    expected.
      ...
      /// <summary>
      ///    Gets or sets a value indicating whether this <see cref="RDotNet.REngine" /> auto print R evaluation results, if they
      ///    are visible.
      /// </summary>
      ...
      /// </summary>
      /// <param name="garbageCollectR">
      ///    if true (default) request an R garbage collection. This happens after the .NET garbage
      ///    collection if both requested
      /// </param>
      /// <param name="garbageCollectDotNet">If true (default), triggers CLR garbage collection and wait for pending finalizers.</param>
      /// <param name="removeHiddenRVars">
      ///    Should hidden variables (starting with '.', such as '.Random.seed') be removed. Default
      ///    is false.
      /// </param>
      /// <param name="detachPackages">
      ///    If true, detach some packages and other attached resources. Default is false. See 'detach'
      ///    function in R
      /// </param>
      /// <param name="toDetach">
      ///    names of resources to dettach, e.g. an array of names such as 'mpg', 'package:lattice'.
      ///    If null, entries found in 'search()' between the first item and 'package:base' are detached. See 'search' function
      ///    documentation in R
      /// </param>
