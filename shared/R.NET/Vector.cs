...
using RDotNet.Internals.PreALTREP;
...
namespace RDotNet
{
   ...
   public abstract class Vector<T> : SymbolicExpression, IEnumerable<T>
   {
      ...
      /// <summary>
      ///    Initializes the content of a vector with runtime speed in mind. This method protects the R vector, then call
      ///    SetVectorDirect.
      /// </summary>
      ...
      /// <summary>
      ///    Gets a representation as a one dimensional array of an R vector, with efficiency in mind for the unmanaged to
      ///    managed transition, if possible.
      /// </summary>
      /// <returns></returns>
      protected abstract T[] GetArrayFast();

      /// <summary>
      ///    Initializes the content of a vector with runtime speed in mind. The vector must already be protected before calling
      ///    this method.
      /// </summary>
      /// <param name="values">The values to put in the vector. Length must match exactly the vector size</param>
      ...
      protected IntPtr DataPointer
      {
         get
         {
            switch (Engine.Compatibility)
            {
               ...
               case REngine.CompatibilityMode.PreALTREP:
                  return IntPtr.Add(handle, Marshal.SizeOf(typeof(VECTOR_SEXPREC)));
               default:
