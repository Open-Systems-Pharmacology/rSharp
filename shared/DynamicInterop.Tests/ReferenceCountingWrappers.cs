...
namespace DynamicInterop.Tests
{
   public class NativeTestLib // : ICustomNativeApi
   {
      ...
      {
         var fileName = PlatformUtility.CreateLibraryFileName("test_native_library");

         const string testLibPathEnv = "DynamicInteropTestLibPath";
         if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable(testLibPathEnv)))
         {
            var guess = GuessTestLibPath();
            if (Directory.Exists(guess))
               Environment.SetEnvironmentVariable(testLibPathEnv, guess);
            else
            {
               //testLibPathEnv = "";
               var msg = "You need to set an environment variable e.g.: " +
                         @"set DynamicInteropTestLibPath=C:\src\dynamic-interop-dll\x64\Debug";
               throw new Exception(msg);
            }
         }

         var nativeLibFilename = PlatformUtility.FindFirstFullPath(fileName, "test_delegate_library DLL", testLibPathEnv);
      ...
      public NativeTestLib()
      {
         if (NativeLib == null)
         {
            ...
            if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable(testLibPathEnv)))
            {
               ...
               {
                  //testLibPathEnv = "";
                  string message = "You need to set an environment variable e.g.: " +
                               @"set DynamicInteropTestLibPath=C:\src\dynamic-interop-dll\x64\Debug";
      ...
      {
         var is64Bits = Environment.Is64BitProcess;

         var baseDir = AppDomain.CurrentDomain.BaseDirectory;
         var diBase = new DirectoryInfo(baseDir);
         var diParent = diBase.Parent;

         var d = baseDir.Trim(Path.DirectorySeparatorChar);
         var baseName = d.Split(Path.DirectorySeparatorChar).Last();
         var dirName = diParent.FullName;
         var maxIterations = 24;
         while (!string.IsNullOrEmpty(baseName) && baseName != "dynamic-interop-dll" && maxIterations > 0)
         {
         ...
         {
            return Path.Combine(d, is64Bits ? "x64" : "Win32", "Debug");
         }
