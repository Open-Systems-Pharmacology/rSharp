...
This page gathers some more technical aspects of the **rSharp** package, and is not meant to be read by most users.
...
```
%R% CMD build rSharp > logbuild.txt 2>&1
%R% CMD check --as-cran rSharp_0.*.tar.gz > logcheck.txt 2>&1
%R% CMD INSTALL --build rSharp_0.*.tar.gz
```

```{r, eval=FALSE}
> setwd('c:/src/codeplex/rSharp')
> tools::buildVignettes(dir='.', tangle=TRUE)
Quitting from lines 224-232 (rSharp_intro.Rmd) 
Error: processing vignette 'rSharp_intro.Rmd' failed with diagnostics:
